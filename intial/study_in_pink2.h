...
public:
    ...
    MapElement *getMovingElement(int r, int c){ return map[r][c];}
};
...
public:
    ...
    {
        if (in_r == r and in_c == c) return true;
        return false;
    };

    bool isEqual(Position p) const
    {
        if (p.getRow() == r and p.getCol() == c) return true;
        return false;
    };
};
class MovingObject
...
public:
    ...
    string str() const
    {
        ...
        return "ArrayMovingObject[count=" + to_string(count) + ";capacity=" + to_string(capacity) + objs + "]";
    };
};
...
public:
    ...
    bool canUse(Character *character, Robot *robot)
    {
        if (character->getName() == "Watson" && character->getHp() % 2 == 0 && robot != nullptr)
        {
            return true;
        }
        return false;
